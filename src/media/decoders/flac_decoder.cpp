...
class __flac_decoder : public dseed::media::media_decoder
{
public:
...
public:
	dseed::error_t initialize (dseed::io::stream* stream)
	{
		...
		auto result = _initFunc (_file,
			...
			{
				dseed::io::stream* stream = ((__flac_decoder*)client_data)->_stream;
			...
			{
				dseed::io::stream* stream = ((__flac_decoder*)client_data)->_stream;
				if (!stream->seek (dseed::io::seekorigin::begin, absolute_byte_offset))
				...
				-> FLAC__StreamDecoderWriteStatus
			{
				...
				dseed::io::stream* stream = jrmdecoder->_stream;
...
public:
	...
	virtual dseed::error_t format (size_t streamIndex, void* buffer, size_t bufferSize) override
	{
		...
		*((dseed::media::audioformat*)buffer) = dseed::media::audioformat (_channels, _bitsPerSample, _samplerate, dseed::media::pulseformat::pcm);
...
public:
	virtual dseed::timespan duration () override
	{
		...
		return dseed::timespan::from_seconds ((bytes * _channels * (_bitsPerSample / 8)) / (double)_byterate);
	}
...
public:
	virtual dseed::timespan reading_position () override
	{
		...
		return dseed::timespan::from_seconds (bytesPos / (double)_byterate);
	}
	...
	virtual dseed::error_t decode (size_t streamIndex, dseed::media::media_sample** sample) override
	{
		...
		if (_initFunc == FLAC__stream_decoder_init_ogg_stream)
			...
			if (!FLAC__stream_decoder_get_decode_position (_file, &bytesPos))
				return dseed::error_fail;
		dseed::timespan current = dseed::timespan::from_seconds (bytesPos / (double)_byterate);
		...
		dseed::timespan duration = dseed::timespan::from_seconds (_bytesRead / (double)_byterate);
		*sample = new __common_media_sample (dseed::media::sampletype_audio, _bytes, current, duration, _byterate);
